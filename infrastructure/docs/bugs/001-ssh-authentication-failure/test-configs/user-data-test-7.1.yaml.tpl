#cloud-config
# Test 7.1: Add reboot configuration (CRITICAL TEST)
# Based on Test 5.1 + reboot after setup
...
# Install packages including UFW
packages:
  - curl
  - wget
  - git
...
# Final message
final_message: |
  Test 7.1 VM setup completed!
  SSH Access: ssh torrust@VM_IP or sshpass -p 'torrust123' ssh torrust@VM_IP

# Power state - reboot after setup - ADDED (CRITICAL TEST)
power_state:
  mode: reboot
